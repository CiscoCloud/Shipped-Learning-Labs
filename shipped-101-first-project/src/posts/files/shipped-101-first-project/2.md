...
#### Modifying our .drone.yml file

Shipped uses Drone for it' Continuous Integration / Continuous Deployment environment. Therefore we will want to view and edit our .drone.yml file to tell Shipped how to build our application.

Issue command "cat main.go" to view the sample main.go file that has been installed for you.

<pre>$ cat .drone.yml</pre></li>
...
git:
  path: github.com/ObjectiveJoe/shipped101helloworld
script:
  - echo "build commands"
...
We want to modify .drone.yml to call our test sript as part of it's build commands. Issue command "vi .drone.yml" to open .drone.yml file in the vi editory
<pre>$ vi .drone.yml</pre></li>

Press 'a' key once to enter edit mode. Script down past the 'script:' tag and change the line '  - /app/bin/test' to be ' ./test.sh 8888'"
